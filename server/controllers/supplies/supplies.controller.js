...
export const getById = _id => {
    ...
    return SupplyModel.findById(itemId)
        ...
        .then(supply => {
            if (!supply) {
                return Promise.reject({ status: 404, message: 'Failed to find supply' });
            }
...
export const getList = data => {
    ...
    return SupplyModel.count(searchQuery)
        ...
        .catch(error => {
            LoggerService.error('Failed to get supplies list. Message:', error.message);
...
export const create = data => {
    return extend(new SupplyModel(), getDataToUpdate(data)).save()
        ...
        .catch(error => {
            LoggerService.error('Failed to create supply. Message:', error.message);
            return Promise.reject(error);
        });
};
...
export const update = (id, data) => {
    ...
    return SupplyModel.findById(itemId)
        ...
        .then(supply => getById(supply._id))
        .catch(error => {
            LoggerService.error('Failed to update supply. Message:', error.message);
            return Promise.reject(error);
        });
};
...
export const remove = id => {
    ...
    return SupplyModel.findById(itemId)
        ...
        .then(supply => supply.save())
        .catch(error => {
            LoggerService.error('Failed to remove supply. Message:', error.message);
            return Promise.reject(error);
        });
};
...
export const updateStatusToShipped = id => {
    ...
    return update(id, { status: 'shipped', shippedAt: Date.now() })
        .catch(error => {
            LoggerService.error('Failed to update supply status (shipped). Message:', error.message);
            return Promise.reject(error);
        });
};
...
export const updateStatusToDeclined = id => {
    ...
    return update(id, { status: 'declined', declinedAt: Date.now() })
        .catch(error => {
            LoggerService.error('Failed to update supply status (declined). Message:', error.message);
            return Promise.reject(error);
        });
};
...
export const updateStatusToReceived = id => {
    ...
    return SupplyModel.findById(itemId)
        ...
        .catch(error => {
            LoggerService.error('Failed to update supply status (received). Message:', error.message);
            return Promise.reject(error);
        });
};
